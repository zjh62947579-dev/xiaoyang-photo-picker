...
**小羊新媒体工具箱** 是一个本地照片筛选与视频素材分类工具。照片流程会把相似照片自动归到“同一个瞬间”，标记明显糊片、闭眼、曝光异常等失败片，再通过 **左右 A/B 擂台 PK** 快速选片；视频流程会按横屏 / 竖屏和风景 / 人像自动整理素材。
...
- 🎬 **视频自动分类**：抽取多个时间点画面识别人像，归档到横屏 / 竖屏与风景 / 人像四类；支持总文件夹批量夜间处理和断点续跑。
...
Windows 启动器会优先使用本机已有的 Python 3.11；如果没有，再尝试 `uv`，之后依次尝试 `winget` 和 python.org 官方安装包安装 Python 3.11。自动下载、Python 运行时、`.venv`、uv、依赖环境和安装缓存默认都放在项目所在盘的 `xiaoyang-photo-picker-runtime`。如果项目在 C 盘，启动器会自动从 D: 到 Z: 选择可用空间最大的本地盘（至少需要约 5GB）；也可以提前设置 `PIANKE_RUNTIME_DIR=E:\xiaoyang-photo-picker-runtime` 手动指定位置。网络受限的电脑也可以先手动安装 Python 3.11，再重新双击启动器。
...
专家模式需要下载 DINOv2、InsightFace、MUSIQ、CLIP-IQA+ 等本地视觉模型，视频分类还会准备约 14 MB 的轻量人体检测模型。普通 GitHub 源码 ZIP 不适合直接塞进 1GB+ 模型文件；如果要给网络受限的朋友使用，建议在已下载成功的电脑上生成“带模型离线包”：
...
- **移动模式（默认，推荐）**：保留的照片继续留在原分类文件夹，淘汰照片移动到 `losers/`。最省磁盘空间，符合一次性选片直觉。反悔时文件会无损搬回原位。
- **复制模式**：原片保持不动，淘汰照片复制到 `losers/`。需要更多磁盘空间。

素材整理后，路线目录通常是 `风景`、`人像`、`合照` 三个原分类文件夹。照片筛选只把不要的照片放进 `losers/`，保留照片不再额外生成 `winners/`。`losers/` 仍优先按淘汰原因分类，并保留原始子文件夹，方便追溯淘汰来源。

## 素材整理工作流

首页切换到“素材整理”后，可先处理喔图等平台导出的路线分类包。程序会识别类似 `无水印_7.18惠州冲浪_风景(已显示&已修未显示).zip` 的文件名，提取路线名和分类名，解压成功后删除原压缩包，并把同一路线归并成：
...
同级目录中 `路线名(1)`、`路线名(2)`、`路线名(3)` 这类重复文件夹会删除；同名文件不会覆盖，会自动追加 `_1`、`_2`。

## 视频分类工作流

首页切换到“视频分类”后，可处理一个素材文件夹，也可以把总文件夹下每个一级子文件夹当作独立路线批量处理。系统读取视频旋转后的实际画面尺寸，并在 7 个时间点抽帧检测人物，不需要完整解码视频。
...
- `风景` 表示抽样画面未检测到人物，`人像` 表示检测到人物；可识别正脸、背影和远景人物，不只依赖人脸。
- 分类文件直接平铺在四个目录中，同名时自动追加 `_1`、`_2`，不保留原文件夹嵌套。
- 图片、文档、封面等非视频文件会统一平铺到 `其他文件/`，避免残留在路线根目录。
- 移动模式节省磁盘空间；复制模式保留原视频，但会额外占用接近相同空间。
- 每完成一个视频就写入本地断点记录。中止或关机后重新启动会跳过已完成项；无法读取的视频留在原位置。
- 人物检测使用项目现有 `torchvision` 依赖中的轻量 SSDLite MobileNetV3，而不是 3B 级视觉语言模型，适合大量视频夜间批量运行。
...
勾选“本地记录筛选偏好”后，每次 PK 选择和初筛召回都会写入匿名训练数据。数据默认只存在本机，不上传照片、不上传真实路径、不上传 EXIF GPS。每个照片文件夹仍保留一份本次记录，同时程序会追加到本机全局训练库，完成页的“导出累计训练数据”会一次性导出所有历史累计数据，便于使用一段时间后统一训练偏好模型。

---

## 相机水印

选片完成后，在保留照片总览页点击 **相机水印** 可以为留下的照片批量添加 EXIF 信息水印。
...
输出到 `watermarked_<时间戳>/` 子目录，保留原 EXIF（旋转标志归位），JPEG 92 质量、progressive。
...
专家模式第一次启动需要从网络下载几百兆到数 GB 的辅助数据。装完之后下次启动会优先使用项目根目录的 <code>models/</code>，可以离线运行。如果另一台电脑下载不了，可以把已成功电脑上的 <code>models/</code> 整个复制到新电脑项目根目录。
</details>
...
<ul>
  <li><b>清理照片缓存</b>：直接删除对应照片目录下的 <code>losers/</code>、<code>review/</code>、<code>.pic_selecter_state.json</code> 及 <code>_pic_selecter/</code> 文件夹；如有历史版本生成的 <code>winners/</code>，确认不再需要后也可删除。</li>
...
1. 极速、专家和视频分类模式下，所有照片与视频**完全在你的电脑里处理**，没有任何素材或数据发往外部。
