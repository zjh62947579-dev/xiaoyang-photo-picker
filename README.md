...
`winners/` 和 `review/` 会先按人脸数量分为 `风景`、`人像`、`合照`：未检测到人脸为风景，1-2 张脸为人像，3 张及以上为合照。保留区会直接把照片放进对应分类目录，不再套原始子文件夹；如遇同名文件会自动追加 `_1`、`_2`。RAW/JPG/XMP 等同名伴随文件会跟随主文件进入同一目录。`losers/` 仍优先按淘汰原因分类，并保留原始子文件夹，方便追溯淘汰来源。
