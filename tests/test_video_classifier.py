...
def test_move_sort_uses_four_requested_categories(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "横屏视频" / "风景" / "a.mov").exists()
    assert (tmp_path / "横屏视频" / "人像" / "b.mov").exists()
    assert (tmp_path / "竖屏视频" / "风景" / "c.mov").exists()
    assert (tmp_path / "竖屏视频" / "人像" / "d.mov").exists()


def test_move_sort_collects_non_video_files_into_other_files(tmp_path, monkeypatch):
    video = tmp_path / "clips" / "one.mp4"
    video.parent.mkdir()
    video.write_bytes(b"video")
    root_image = tmp_path / "cover.jpg"
    nested_doc = tmp_path / "notes" / "info.txt"
    nested_doc.parent.mkdir()
    root_image.write_bytes(b"cover")
    nested_doc.write_text("note", encoding="utf-8")
    (tmp_path / video_classifier.STATE_FILENAME).write_text("{}", encoding="utf-8")
    ...
    assert summary.other_files == 2
    assert not root_image.exists()
    assert not nested_doc.exists()
    assert (tmp_path / "其他文件" / "cover.jpg").read_bytes() == b"cover"
    assert (tmp_path / "其他文件" / "info.txt").read_text(encoding="utf-8") == "note"
    assert (tmp_path / video_classifier.STATE_FILENAME).exists()
...
def test_video_status_payload_includes_other_files(tmp_path, monkeypatch):
    app = import_app_module()
    job = app.VideoJobState(folder=str(tmp_path), other_files=3)
    monkeypatch.setattr(app, "VIDEO_JOB", job)

    payload = app._video_job_payload()

    assert payload["other_files"] == 3
