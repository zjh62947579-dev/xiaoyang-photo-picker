...
分成风景/人像。分类结果保存在每个素材文件夹的 ``视频分类`` 目录。
"""
...
def state_path(folder: str | Path) -> Path:
    return output_dir(folder) / STATE_FILENAME
    ...
    *,
    ...
    for index, source in enumerate(videos, start=1):
        ...
        try:
            ...
            target_folder = output_dir(root) / analysis.orientation / analysis.content
